...
f_in=glob.glob(PATH_TO_JSON+'/chunk*.json')

def load_json():
    #json_array=[]
    for chunk_name in f_in:
        with  open(chunk_name,'r') as chunk
            while 1:
                try:
                    entry=json.load(chunk)
                    yield entry
    #                json_arry.append(entry)
                except Exception as e:
                    break

text_array=[]
for page_json in load_json():
    text=[]
    for each_text in page_json['text']:
        text.extend(each_text)
        text.extend(' ')
